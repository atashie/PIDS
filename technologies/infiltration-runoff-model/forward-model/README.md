...
| 2026-06-04 | **Module 1 (subsurface) BUILT + three-tier sanity-checked** (TDD). Mixed-form Richards; van Genuchten–Mualem + **Vogel/Ippisch air-entry** (fixes the Mualem-K saturation singularity); **mass-lumped** storage (suppresses wetting-front oscillation); adaptive backward-Euler cut-and-retry; dimension-agnostic 1-D/2-D/3-D; BCs = Dirichlet / Neumann flux / **surface ponding store** (rain excess raises the head — vertical accumulation only, lateral routing deferred to Module 2). MMS spatial 2nd-order + temporal 1st-order; conservation to ~1e-12; 20 tests green. Modeling assumptions locked: **unconfined aquifer, no specific-storage `Ss`** (compressibility ignored), no solute transport (see design §B.2/§B.3). Two adversarial reviews (multi-agent + Codex) applied. | Engine-internal. `parameterization` will supply per-layer van Genuchten + field-capacity params when soils become heterogeneous. No interface break. |