...
def test_lake_at_rest_is_held_exactly_1d():
    """A still pond over a SLOPING bed must stay EXACTLY at rest (well-balanced gate).

    Initial surface head H = z_b + d is uniform, so every edge head difference H_i - H_j is
    zero and the upwind edge flux Q_e = T_e M(d_up)(H_i - H_j) vanishes STRUCTURALLY (not just
    to eps_S / eps_H tolerance -- the scheme differences H, not d, so this is exact regardless
    of the regularization widths; the gate cannot be tuned to pass). The depth field must not
    move and no spurious flux may run down the bed slope.
    """
...
def test_nonflat_head_drives_flow_1d():
    ...
    in a closed domain. Guards against shipping an "at-rest-for-everything" class.
    """
    L = 10.0
