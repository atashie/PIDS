...
> **UPDATE 2026-06-05 — MODULE 2 BUILT, Tier-3 SIGNED OFF (Arik). Two amendments to the spec below, authoritative over the original §C.4/§C.8/§H.2 text:**
> 1. **Depth-positivity pivot (revises §C.4 "VI primary", §C.8, §H.2):** adversarial Tier-1 testing proved PETSc **SNES-VI (`vinewtonrsls`, also `vinewtonssls`; raw AND via DOLFINx 0.10 `NonlinearProblem`; all line searches) CANNOT robustly solve the stiff, nonsymmetric diffusion-wave** — it walls within 3–15 steps and violates conservation when the bound binds (the install-spike VI success was a *linear symmetric* system and did not transfer). **Shipped instead:** smooth `newtonls`/bt headline solve + a **post-step CONSERVATIVE positivity limiter** (clip `d<0`→0, rescale remaining positive water by `oldtotal/posvol`; no-op when no negatives; degenerate non-positive-total state is dried with the tiny unavoidable adjustment tracked). **VI is retired for overland** — and the §D coupling monolith no longer needs a VI path for overland depth-positivity. (decision-log 2026-06-05.)
> 2. **Synoptic verification result:** the implemented diffusion-wave reproduces kinematic Manning behavior across an 8-scenario matrix — every run reaches `q_eq = r·L` (continuity), and peak outflow vs peak velocity follow `q ∝ u^{5/2}` at fixed `n,S` (since `q=u·d`, `u∝d^{2/3}`, `q∝d^{5/3}`): regime (a) vary-rain → `q∝r` linear, `u∝q^{2/5}`; regime (b) vary-terrain → `q` constant, `u∝S^{3/10}n^{-3/5}`. Power-law ratios match to ~2 sig figs (see `validation/sanity/overland__2026-06-05.md`).
>
> Status: Tier-1 (13 tests) + Tier-2 (5) + adversarial review + Tier-3 human sign-off all PASS; full suite 39/39. **NEXT = Module 3 (coupling, §D).**
...
> **NOTE 2026-06-05 (Module 3 is NEXT; read before building):** Module 1 (subsurface) and Module 2 (overland) are COMPLETE + Tier-3 signed off. Two carry-over consequences for this section: (1) **overland depth-positivity is NO LONGER a VI bound** — it is a smooth `newtonls` solve + a post-step conservative limiter (§C update 2026-06-05), so the monolithic solve here is **plain SNES Newton with no VI block for `d≥0`**; the limiter applies as a post-Newton-step correction on the `d` DOFs (decide its placement in the coupled loop — likely after the monolithic step, before acceptance, conserving each block). (2) Module 2 was built **standalone on its own surface mesh**; coupling must realize `d` on the **host top facets** (or a shared co-dim-1 submesh) and assemble the land-surface exchange `q_ls = k_ex·(d − ψ_top)` (Robin, default B, D.3) into the blocked `[ψ, d]` residual — this is the first place the R1 cross-mesh `ufl.derivative` assumption is exercised at the host↔surface interface (M0 spike territory). The exchange terms `q_ls`/`q_feat` referenced below as §B/§E "hooks" are exposed: subsurface `add_*`/`Q_exchange` slot (§B.7) and overland `add_rain`/source + the `H_s`/`d`/`outflow_rate` accessors (`pids_forward/physics/overland.py`).
