...
**Dry-branch regularization (NUMERICS DECISION, not just damping).** `dK/dSe` (hence `dK/dψ`, required for the **exact UFL Jacobian** and for the off-diagonal exchange linearization through `σ_soil = K(ψ)·Ω_geom`) is **singular as `Se → 0⁺`**: the Mualem bracket-derivative contains `(1−Se^{1/m})^{m−1}·Se^{1/m−1}` with `1/m − 1 < 0` for `n ∈ (1,2)`, so it diverges at residual saturation. On a **bone-dry antecedent** (a mandatory Tier-2 end-member) `ufl.derivative` faithfully reproduces this divergence and stalls Newton. Fix: **clamp effective saturation to `Se ∈ [Se_min, 1]` with `Se_min ≈ 1e-3…1e-2`** (a smooth `Se = max_soft(Se, Se_min)`), optionally backed by a **Kirchhoff transform** for the dry branch (Q16). This bounds both `dK/dψ` and the exchange-Jacobian `∂σ_soil/∂ψ`, keeping the feature-exchange linearization finite over dry soil. The floor enters the constitutive law itself, so it is applied identically on the headline path and in the analytical/MMS tests.

### B.3 Primary variable & saturated/unsaturated transition

`h` is the single unknown everywhere (no regime switching), keeping saturated (`h≥0, K=Ks, C=0`) and unsaturated (`h<0`) zones in **one continuous formulation** — essential because PIDS features straddle the water table (below it a feature *receives* groundwater and drains laterally; over dry soil the same feature *disperses* at the matric-potential rate). The hazard is `C(h)→0` as `h→0+`: the saturated block degenerates toward Laplace/incompressibility. Mitigations: (a) small specific-storage `Ss·Se·dh/dt` in the saturated branch; (b) **mass-lumped** capacity (B.4); (c) line-search / pseudo-transient continuation. **SNES-VI is NOT used in Module 1** — reserved for overland depth-positivity (§C) and optional one-way devices (§E).

**Provisional `Ss` decision (firms Q13 enough to define the conservation test).** Include a **physical** specific-storage term `Ss·Se·∂h/∂t` in the saturated branch (SE-Piedmont saprolite/residuum `Ss ≈ 1e-5…1e-4 m⁻¹`, value confirmed in Q13). Because it is treated as physical, **`Ss` storage enters the storage budget**: the Tier-1 conservation gate is `|Δ(θ-storage + Ss·Se·h storage) − net_flux − sources|/scale < 1e-6`, not the `θ`-only budget. (If a later study shows the term is needed purely as numerical regularization, it is shrunk toward zero and dropped from the budget — but the test is written to the physical-`Ss` accounting now so it does not silently miss saturated-zone storage.)
...
J = derivative(F, h)                 # symbolic Jacobian -> PETSc SNES (Se floored so dK/dψ is finite)
```
